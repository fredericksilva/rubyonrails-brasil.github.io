...
module MiddlemanCasperHelpers
  ...
  def blog_author
    if is_author_page?
      find_author(current_resource.metadata[:locals][:current_author])
    elsif is_blog_article?
      find_author(current_article.data.author)
    elsif @article
      find_author(@article.data.author)
    else
      find_author
    end
  end

  def find_author(author_slug=nil)
    if author_slug
      ...
      if result.any?
        ...
        return OpenStruct.new(resource.merge(slug: resource.keys.first))
      end
    end
  ...
  def author_path
    "#{blog.options.prefix.to_s}/author/#{blog_author.slug}/"
  end
